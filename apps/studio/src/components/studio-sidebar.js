...
export function StudioSidebar() {
    ...
    return (_jsx("aside", { className: cn('fixed left-0 top-16 z-40 h-[calc(100vh-4rem)] bg-theme-primary border-r border-theme transition-all duration-300', collapsed ? 'w-16' : 'w-60'), children: _jsxs("nav", { className: "h-full flex flex-col", children: [_jsx("div", { className: "flex-1 overflow-y-auto py-4", children: _jsx("ul", { className: "space-y-1 px-2", children: navigation.map((item) => {
                            ...
                            return (_jsxs("li", { children: [_jsxs("div", { className: "flex items-center", children: [_jsxs(Link, { to: item.to, className: cn('flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors flex-1', isActive
                                                    ...
                                                    : 'hover:bg-theme-secondary'), children: [_jsx(Icon, { className: "h-5 w-5 flex-shrink-0" }), !collapsed && _jsx("span", { children: item.name })] }), !collapsed && item.subItems.length > 0 && (_jsx("button", { onClick: () => toggleExpand(item.name), className: "p-1 hover:bg-theme-secondary rounded transition-colors", children: _jsx(ChevronDown, { className: cn('h-4 w-4 transition-transform', isExpanded && 'rotate-180') }) }))] }), !collapsed && isExpanded && (_jsx("ul", { className: "mt-1 ml-4 space-y-1", children: item.subItems.map((subItem) => (_jsx("li", { children: _jsxs(Link, { to: subItem.to, className: "flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-theme-secondary rounded-md transition-colors", children: [_jsx("span", { className: "flex-1", children: subItem.name }), _jsx(KBBadge, { variant: "default", className: "text-[10px]", children: "Soon" })] }) }, subItem.to))) }))] }, item.name));
                        }) }) }), _jsx("div", { className: "border-t border-theme p-2", children: _jsx("button", { onClick: toggleCollapse, className: "flex items-center justify-center w-full px-3 py-2 text-sm font-medium hover:bg-theme-secondary rounded-lg transition-colors", children: collapsed ? (_jsx(ChevronRight, { className: "h-5 w-5" })) : (_jsxs(_Fragment, { children: [_jsx(ChevronLeft, { className: "h-5 w-5" }), _jsx("span", { className: "ml-2", children: "Collapse" })] })) }) })] }) }));
}
